...
for(i=0; i< myDocument.pages.length ; i++ )
{
	...
	for(j=0 ; j< myDocument.pages[i].allGraphics.length ; j++)
	{
		...
		for(k=0;k< myDocument.pages[i].textFrames.length ; k++)
		{
			...
			// if the text frame falls within the graphic, remember the text frame
			...
			// if there are any saved text frames
			if(inner_tf.length > 0)
			{
				// create a group of these text grames and the surrounding graphic
